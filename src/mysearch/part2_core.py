"""
Core indexing/search logic for Part 2 — Strict Boolean & Phrase Search
...
def tokenize(text: str) -> List[str]:
    """Extract alphabetic tokens and normalize them."""
    return [_norm(w) for w in re.findall(r"[A-Za-z']+", text) if w]
...
def build_index_from_zip(zip_path: str) -> InvertedIndex:
    ...
    with zipfile.ZipFile(zip_path, "r") as z:
        ...
        for name in z.namelist():
            ...
            # Strip HTML tags (simple regex-based)
            text = re.sub(r"<[^>]+>", " ", html)
            ...
            for w, positions in pos_map.items():
                if w not in inv.inv:
                    inv.inv[w] = {}
                inv.inv[w][doc_id] = Posting(freq=len(positions), positions=positions)
    ...
    for term, postings in inv.inv.items():
        ...
        for doc_id, post in postings.items():
            ...
            inv.docs[doc_id].norm += post.tfidf ** 2
...
def boolean_and(inv: InvertedIndex, terms: List[str]) -> Set[int]:
    ...
    # Sort terms by frequency (smallest postings first for efficiency)
    terms_sorted = sorted((_norm(t) for t in terms), key=lambda x: len(inv.postings(x)))
    if not inv.postings(terms_sorted[0]):
        return set()

    result = set(inv.postings(terms_sorted[0]).keys())
...
def _adjacent_positions(a: List[int], b: List[int]) -> bool:
    """Check if any position in b is exactly +1 after a position in a."""
    i = j = 0
...
# ---------------------------
# Vector Space Ranking (Q4)
# ---------------------------

def vector_rank(inv: InvertedIndex, query: str, topk=30) -> List[Tuple[int, float]]:
